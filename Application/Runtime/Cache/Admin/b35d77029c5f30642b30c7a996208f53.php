...
<?php else: ?>
    ...
    <table>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="name"><?php echo ($vo['role']); ?></td>
                <td class="process">
                    <?php if($vo['auth_ids'] == '*'): ?>最高权限无需分配
                    <?php else: ?>
                        <a href="/index.php/Admin/Role/auth/auth_ids/<?php echo ($vo['auth_ids']); ?>/role/<?php echo ($vo['role']); ?>/id/<?php echo ($vo['id']); ?>">分配权限</a><?php endif; ?>
                </td>
                <td class="operate">
                    <a href="<?php echo U('Admin/Doc/edit',array('id'=>$vo['id']));?>" >编辑</a> |
                    <!--<a href="/index.php/Admin/Role/del/id/<?php echo ($vo["id"]); ?>" onclick="return confirm('确定要删除吗？')" >删除</a>-->
...
$('.check_content').bind('click',function(){
    ...
    var index;
    $.ajax({
        url:"/index.php/Admin/Role/checkContent",
        type:'post',
        data:{id:id},
        dataType:'json',
        beforeSend:function(){
