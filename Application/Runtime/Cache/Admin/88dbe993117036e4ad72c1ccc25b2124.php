...
<?php else: ?>
    ...
    <table>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="node" >
                    <?php if($vo['thumb_img'] == 0): ?>无图片
                    <?php else: ?>
                        <div class="zoom-small-image">
                            <a href="/uploads/<?php echo ($vo['ori_img']); ?>" class = 'cloud-zoom' id='zoom1' rel="adjustX:10, adjustY:-4">
                                <img src="/uploads/<?php echo ($vo['thumb_img']); ?>" height="150" alt='' />
                            </a>
                        </div><?php endif; ?>
                </td>
                <td class="time"><?php echo (date("Y-m-d h:i:s",$vo['add_time'])); ?></td>
                <td class="operate">
                    <!--<a href="/index.php/Admin/Knowledge/del/id/<?php echo ($vo["id"]); ?>" onclick="return confirm('确定要删除吗？')" >删除</a>-->
                    <a href="javascript:void(0)" class="ajax-delete" thumb_img="<?php echo ($vo['thumb_img']); ?>" ori_img="<?php echo ($vo['ori_img']); ?>" id="<?php echo ($vo["id"]); ?>" >删除</a>
                </td>
...
$('.check_content').bind('click',function(){
    ...
    var index;
    $.ajax({
        url:"/index.php/Admin/Knowledge/checkContent",
        type:'post',
        data:{id:id},
        dataType:'json',
        beforeSend:function(){
...
$('.ajax-delete').bind('click',function(){
    ...
    var me = this;
    var id = $(me).attr('id');
    var thumb_img = $(me).attr('thumb_img');
    var ori_img = $(me).attr('ori_img');
    if(confirm('确定要删除吗？')){
        $.ajax({
            type:'POST',
            url:'/index.php/Admin/Knowledge/del',
            data:{id:id,thumb_img:thumb_img,ori_img:ori_img},
            dataType:'json',
            success: function(data){
                console.log(data);
                if(data.code == 1){
                    ...
                    if(lines > 0){
                       ...
                       // $("body").empty().append('<div class="title"><h2>信息管理</h2></div> <div class="">对不起，没有数据！</div>');
                        window.location.href="/index.php/Admin/Knowledge/index";
                    }
